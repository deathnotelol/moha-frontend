...
//   const placeholderImg = "/images/Web.jpg"; // Default image
...
//       const rawImages = Array.isArray(res.data.all_images)
//         ? res.data.all_images
//         : [];
...
//                     <div
//                       key={idx}
//                       className="flex justify-center bg-black/5 p-2"
//                     >
//                       <img
//                         src={
//                           img === placeholderImg
...
const PostDetail = () => {
  ...
  const [post, setPost] = useState(null);
  const [images, setImages] = useState([]);
  const placeholderImg = process.env.PUBLIC_URL + "/images/Web.jpg";
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const res = await api.get(`/posts/${id}`); // <-- id ကို UUID ဖြင့်ယူရမယ်
        setPost(res.data.mm); // Backend က mm property ထဲမှာ post object
        const rawImages = Array.isArray(res.data.mm.all_images)
          ? res.data.mm.all_images
          : [];
        const uniqueImages = [
          ...new Set(
            rawImages.map((img) =>
              img.split("#")[0].replace(/%20/g, " ").trim()
            )
          ),
        ];
        setImages(uniqueImages.length === 0 ? [placeholderImg] : uniqueImages);
      } catch (err) {
        console.error("Failed to fetch post", err);
        setImages([placeholderImg]);
      }
